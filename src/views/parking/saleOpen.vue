        ...
        <div v-for="(value, key, index) in list" :key="index" class="parking-checked-group">
            ...
            <p class="el-checkbox-group1">
                <label v-for="item in value" :key="item.parking_id" :style="item.disabled === true ? 'color:#cccccc;' : ''">
                    <input type="checkbox" :disabled="item.disabled" :value="item.parking_no">
                     {{ item.parking_no }}
                </label>
        ...
        methods: { 
            ...
            deleteAll() {
                ...
                this.$confirm('确认批量删除车位？')
                    .then(_=> {
                        httpClient.post('admin/parking/delete-all', {parking_no: temp,project_id:this.project_id}).then(response => {
